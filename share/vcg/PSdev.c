...
#ifndef lint
static char *id_string="$Id: PSdev.c,v 1.2 2009/12/17 17:07:29 frb Exp $";
#endif
...
#endif
{
	...
	FPRINTF(fptr,"NP %ld %ld M %ld %ld P %ld %ld P %ld %ld P CP fill\n",
...
#endif
{
	...
	case LEFT_TO_RIGHT:
		FPRINTF(fptr,"NP %ld %ld M %ld %ld P %ld %ld P CP fill\n",
			x-V_xmin+w  , dist - (y-V_ymin),
			x-V_xmin+w  , dist - (y-V_ymin+h),
			x-V_xmin    , dist - (y-V_ymin+h/2));
		return;
...
#endif
{
	...
	FPRINTF(fptr,"%d LW ",t);
        FPRINTF(fptr,"%ld %ld M %ld %ld D 1 LW\n",
                fx-V_xmin, dist - (fy-V_ymin), 
		tx-V_xmin, dist - (ty-V_ymin));
}
...
#endif
{
	...
	FPRINTF(fptr,"[ %d %d ] 0 SD %d LW ",d,d,t);
        FPRINTF(fptr,"%ld %ld M %ld %ld D [ 100 0 ] 0 SD 1 LW\n",
                fx-V_xmin, dist - (fy-V_ymin), 
		tx-V_xmin, dist - (ty-V_ymin));
}
...
#endif
{
	...
	case UNVISIBLE:
        ...
        FPRINTF(fptr,"%ld %ld M %ld %ld ",
                x0-V_xmin, dist - (y0-V_ymin), 
		x1-V_xmin, dist - (y1-V_ymin));
        FPRINTF(fptr,"%ld %ld %ld %ld curveto stroke\n",
