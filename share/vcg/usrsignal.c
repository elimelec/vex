...
#ifndef lint
static char *id_string="$Id: usrsignal.c,v 1.2 2009/12/17 17:07:30 frb Exp $";
#endif
...
#endif
	disable_signal();
	if ( signal(SIGUSR1,(void (*)())animation_handler) == SIG_ERR ) {
        	FPRINTF(stderr,"Can't set USR1 process signal.\n");
		exit(-1);  /* not gs_exit, because the device is not there */
	}
	if ( signal(SIGUSR2,(void (*)())windowclose_handler) == SIG_ERR ) {
