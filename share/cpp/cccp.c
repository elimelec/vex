...
static char sccs_id[] = "$Id: cccp.c,v 1.5 2006/07/14 10:29:31 frb Exp $ ";
...
#ifdef VEXCPP
       /* put the directive back at the beginning of the line */ 
       {
           U_CHAR *p = obp-1;
           while (*p == ' ' | *p == '\t') p--;
           if (*p == '\n')
...
void warning_with_line (int line, PRINTF_ALIST (msg))
{
  va_list args;
...
void pedwarn_with_line (int line, PRINTF_ALIST (msg))
{
  va_list args;
