...
## $Id: as-filter.sh,v 1.1 2004/10/20 10:25:24 frb Exp $
...
sed -r -e 's/sim_icache_fetch/sim_icache_fetch_filter/' \
    -e 's/(#define [_]*GOTO\(.* )/\1 sim_branch_filter(0,#x)/' \
    -e 's/(#define [_]*BRANCHF\(.* )/\1 sim_branch_filter(2,#x)/' \
    -e 's/(#define [_]*BRANCH\(.* )/\1 sim_branch_filter(1,#x)/' \
    -e 's/(#define [_]*CALL\(.* )/\1 sim_branch_filter(3,#x)/' \
    -e 's/(#define [_]*RETURN\(.* )/\1 sim_branch_filter(4,#x)/' \
    -e 's/(#define [_]*ENTRY\(.* )/\1 sim_branch_filter(5,#x)/' \
    -e 's/(#define [_]*LABEL\(.* )/\1 sim_branch_filter(6,#x)/' \
    $tmp_cs_files.c > $tmp_cs_files.filter.cs.c
