/* 
** static char sccs_id[] = "$Id: malloc.c,v 1.2 2003/05/21 12:25:29 frb Exp $ ";
*/
...
_PTR
_DEFUN_WRAP (malloc, (nbytes),
	size_t nbytes)		/* get a block */
{
  _PTR retval;
  THREAD_LOCK();
  retval = _malloc_r (_REENT, nbytes);
  THREAD_UNLOCK();
  return retval;
}

void
_DEFUN_WRAP (free, (aptr),
	_PTR aptr)
{
  THREAD_LOCK();
  _free_r (_REENT, aptr);
...
_PTR
_DEFUN_WRAP (realloc, (ap, nbytes),
	_PTR ap _AND
	size_t nbytes)
{
  _PTR retval;
  THREAD_LOCK();
  retval = _realloc_r (_REENT, ap, nbytes);
  THREAD_UNLOCK();
  return retval;
}
