...
void Configure(int ac, char *av[])
{
  ...
  {
    snprintf(errortext, ET_SIZE, "Error1 open file %s!",TRACEFILE);
    error(errortext,500);
  }
#endif
  ...
  {
    snprintf(errortext, ET_SIZE, "Error2 open file %s ",params->outfile);
    error(errortext,500);
  }
...
#define OUTSTRING_SIZE 255
  ...
  if ((p_log=fopen(string,"r"))==0)                    // check if file exist
  {
    ...
    {
      snprintf(errortext, ET_SIZE, "Error3 open file %s for appending",string);
      error(errortext, 500);
    }
