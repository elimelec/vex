...
static void init_picture(ImageParameters *img, Slice *currSlice, struct inp_par *inp)
{
  ...
  if (seiToneMapping.seiHasTone_mapping)
  {
    ...
    if (NULL == (void*)dec_picture->tone_mapping_lut)
    {
              ...
              int *p_ref)              //!< open reference YUV file
{
  ...
  max_pix_value_sqd[0] = iabs2(img->max_imgpel_value_comp[0]);
  max_pix_value_sqd[1] = iabs2(img->max_imgpel_value_comp[1]);
  max_pix_value_sqd[2] = iabs2(img->max_imgpel_value_comp[2]);
  cur_ref[0] = imgY_ref;
  cur_ref[1] = p->chroma_format_idc != YUV400 ? imgUV_ref[0] : NULL;
  cur_ref[2] = p->chroma_format_idc != YUV400 ? imgUV_ref[1] : NULL;
  cur_comp[0] = p->imgY;
  ...
  framesize_in_bytes = (( comp_size_x[0] * comp_size_y[0]) + ( comp_size_x[1] * comp_size_y[1] ) * 2) * symbol_size_in_bytes;
...
void set_ref_pic_num(void)
{
  ...
  {
    dec_picture->ref_pic_num        [slice_id][LIST_0][i] = __c64(0,listX[LIST_0][i]->poc * 2 + ((listX[LIST_0][i]->structure==BOTTOM_FIELD)?1:0)) ;
    dec_picture->frm_ref_pic_num    [slice_id][LIST_0][i] = __c64(0,listX[LIST_0][i]->frame_poc * 2);
    dec_picture->top_ref_pic_num    [slice_id][LIST_0][i] = __c64(0,listX[LIST_0][i]->top_poc * 2);
    dec_picture->bottom_ref_pic_num [slice_id][LIST_0][i] = __c64(0,listX[LIST_0][i]->bottom_poc * 2 + 1);
  ...
  {
    dec_picture->ref_pic_num        [slice_id][LIST_1][i] = __c64(0,listX[LIST_1][i]->poc  *2 + ((listX[LIST_1][i]->structure==BOTTOM_FIELD)?1:0));
    dec_picture->frm_ref_pic_num    [slice_id][LIST_1][i] = __c64(0,listX[LIST_1][i]->frame_poc * 2);
    dec_picture->top_ref_pic_num    [slice_id][LIST_1][i] = __c64(0,listX[LIST_1][i]->top_poc * 2);
    dec_picture->bottom_ref_pic_num [slice_id][LIST_1][i] = __c64(0,listX[LIST_1][i]->bottom_poc * 2 + 1);
  }
  ...
  if (!active_sps->frame_mbs_only_flag)
  {
    if (img->structure==FRAME)
    {
      for (j=2;j<6;j++)
      {
        ...
        {
          dec_picture->ref_pic_num        [slice_id][j][i] = __c64(0,listX[j][i]->poc * 2 + ((listX[j][i]->structure==BOTTOM_FIELD)?1:0));
          dec_picture->frm_ref_pic_num    [slice_id][j][i] = __c64(0,listX[j][i]->frame_poc * 2) ;
          dec_picture->top_ref_pic_num    [slice_id][j][i] = __c64(0,listX[j][i]->top_poc * 2) ;
          dec_picture->bottom_ref_pic_num [slice_id][j][i] = __c64(0,listX[j][i]->bottom_poc * 2 + 1);
        }
      }
    }
  }
...
void copy_dec_picture_JV( StorablePicture *dst, StorablePicture *src )
{
  ...
  if( src->tone_mapping_lut )
  {
    ...
    if (NULL == (void*)dst->tone_mapping_lut)
    {
