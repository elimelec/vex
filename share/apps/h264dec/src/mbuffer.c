...
void gen_field_ref_ids(StorablePicture *p)
{
  ...
  for (i=0 ; i<p->size_x/4 ; i++)
  {
    for (j=0 ; j<p->size_y/4 ; j++)
    {
        ...
        p->motion.ref_id[LIST_0][j][i] = __c64(0,(dummylist0>=0)? p->ref_pic_num[p->slice_id[j>>2][i>>2]][LIST_0][dummylist0] : 0);
        p->motion.ref_id[LIST_1][j][i] =__c64(0, (dummylist1>=0)? p->ref_pic_num[p->slice_id[j>>2][i>>2]][LIST_1][dummylist1] : 0);
  ...
  if (!frame->MbaffFrameFlag)
  {
    for (j = 0; (j < frame->size_y >> 2) ; j++)
    {
      ...
      for (i = 0 ; i < (frame->size_x >> 2) ; i++)
      {
        ...
        frame->motion.ref_id[LIST_0][j][i] = (dummylist0>=0)? frame->ref_pic_num[frame->slice_id[jdiv][idiv]][LIST_0][dummylist0] : __c64(-1,-1);
        frame->motion.ref_id[LIST_1][j][i] = (dummylist1>=0)? frame->ref_pic_num[frame->slice_id[jdiv][idiv]][LIST_1][dummylist1] : __c64(-1,-1);
      }
    }
  }
  else
    ...
    for (j = 0; (j < frame->size_y >> 2) ; j++)
    {
      ...
      for (i = 0 ; i < (frame->size_x >> 2) ; i++)
      {
        ...
        if (frame->motion.mb_field[currentmb])
        {
          ...
          dummylist1 = frame->motion.ref_idx[LIST_1][j][i];
          //! association with id already known for fields.
          frame->motion.ref_id[LIST_0 + list_offset][j][i] = (dummylist0>=0)? frame->ref_pic_num[frame->slice_id[jdiv][idiv]][LIST_0 + list_offset][dummylist0] : __c64(0,0);
          frame->motion.ref_id[LIST_1 + list_offset][j][i] = (dummylist1>=0)? frame->ref_pic_num[frame->slice_id[jdiv][idiv]][LIST_1 + list_offset][dummylist1] : __c64(0,0);
          //! need to make association with frames
          frame->motion.ref_id[LIST_0][j][i] = (dummylist0>=0)? frame->frm_ref_pic_num[frame->slice_id[jdiv][idiv]][LIST_0 + list_offset][dummylist0] : __c64(0,0);
          frame->motion.ref_id[LIST_1][j][i] = (dummylist1>=0)? frame->frm_ref_pic_num[frame->slice_id[jdiv][idiv]][LIST_1 + list_offset][dummylist1] : __c64(0,0);
      ...
      for (i=0 ; i < (frame->size_x >> 2) ; i++)
      {
        ...
        if (!frame->MbaffFrameFlag  || !frame->motion.mb_field[currentmb])
        {
          ...
          else
          {
            ...
            fs_top->motion.ref_id[LIST_0][j][i] = (dummylist0>=0)? frame->top_ref_pic_num[frame->slice_id[jj>>2][ii>>2]][LIST_0][dummylist0] : __c64(0,0);
            fs_btm->motion.ref_id[LIST_0][j][i] = (dummylist0>=0)? frame->bottom_ref_pic_num[frame->slice_id[jj>>2][ii>>2]][LIST_0][dummylist0] : __c64(0,0);
          }
          ...
          else
          {
            ...
            fs_top->motion.ref_id[LIST_1][j][i] = (dummylist1>=0)? frame->top_ref_pic_num[frame->slice_id[jj>>2][ii>>2]][LIST_1][dummylist1] : __c64(0,0);
            fs_btm->motion.ref_id[LIST_1][j][i] = (dummylist1>=0)? frame->bottom_ref_pic_num[frame->slice_id[jj>>2][ii>>2]][LIST_1][dummylist1] : __c64(0,0);
          }
        }
        else
        {
...
void dpb_combine_field(FrameStore *fs)
{
  ...
  for (j=0; j<=(imax(fs->top_field->max_slice_id, fs->bottom_field->max_slice_id)); j++)
  {
    for (k = LIST_0; k <= LIST_1; k++)
    {
      for (i=0;i<16;i++)
      {
        fs->frame->ref_pic_num[j][k][i]=
            i64min(
                __shl64(__shr64(fs->top_field->ref_pic_num[j][k][2*i],1),1),
                __shl64(__shr64(fs->bottom_field->ref_pic_num[j][k][2*i],1),1) );
      }
    }
  }
  ...
  for (j=0 ; j < (fs->top_field->size_y >> 2) ; j++)
  {
    ...
    for (i=0 ; i< (fs->top_field->size_x >> 2) ; i++)
    {
      ...
      //! association with id already known for fields.
      fs->top_field->motion.ref_id[LIST_0][j][i] = (dummylist0>=0)? fs->top_field->ref_pic_num[fs->top_field->slice_id[j>>2][i>>2]][LIST_0][dummylist0] : __c64(0,0);
      fs->top_field->motion.ref_id[LIST_1][j][i] = (dummylist1>=0)? fs->top_field->ref_pic_num[fs->top_field->slice_id[j>>2][i>>2]][LIST_1][dummylist1] : __c64(0,0);

      //! need to make association with frames
      fs->frame->motion.ref_id[LIST_0][jj][i] = (dummylist0>=0)? fs->top_field->frm_ref_pic_num[fs->top_field->slice_id[j>>2][i>>2]][LIST_0][dummylist0] : __c64(0,0);
      fs->frame->motion.ref_id[LIST_1][jj][i] = (dummylist1>=0)? fs->top_field->frm_ref_pic_num[fs->top_field->slice_id[j>>2][i>>2]][LIST_1][dummylist1] : __c64(0,0);
      ...
      fs->bottom_field->motion.ref_id[LIST_0][j][i] = (dummylist0>=0)? fs->bottom_field->ref_pic_num[fs->bottom_field->slice_id[j>>2][i>>2]][LIST_0][dummylist0] : __c64(0,0);
      fs->bottom_field->motion.ref_id[LIST_1][j][i] = (dummylist1>=0)? fs->bottom_field->ref_pic_num[fs->bottom_field->slice_id[j>>2][i>>2]][LIST_1][dummylist1] : __c64(0,0);

      //! need to make association with frames
      fs->frame->motion.ref_id[LIST_0][jj4][i] = (dummylist0>=0)? fs->bottom_field->frm_ref_pic_num[fs->bottom_field->slice_id[j>>2][i>>2]][LIST_0][dummylist0] : __c64(-1,-1);
      fs->frame->motion.ref_id[LIST_1][jj4][i] = (dummylist1>=0)? fs->bottom_field->frm_ref_pic_num[fs->bottom_field->slice_id[j>>2][i>>2]][LIST_1][dummylist1] : __c64(-1,-1);
      ...
      for (i=0 ; i<fs->size_x/4 ; i++)
      {
        ...
        if (!img->MbaffFrameFlag )
        {
          ...
          if (p_frm_motion->ref_idx[LIST_0][jj][ii] == -1)
          {
            ...
            p_motion->ref_pic_id[LIST_0][j][i] = __c64(-1,-1);
          }
          else
          {
          ...
          if (p_frm_motion->ref_idx[LIST_1][jj][ii] == -1)
          {
            ...
            p_motion->ref_pic_id[LIST_1][j][i] = __c64(-1,-1);
          }
          else
          {
      ...
      for (i=0 ; i<fs->size_x/4 ; i++)
      {
        ...
        if (!img->MbaffFrameFlag )
        {
          ...
          if (fs->JVmotion[np].ref_idx[LIST_0][jj][ii] == -1)
          {
            ...
            p->frame.ref_pic_id[LIST_0][j][i] = __c64(-1,-1);
          }
          else
          {
          ...
          if (fs->JVmotion[np].ref_idx[LIST_1][jj][ii] == -1)
          {
            ...
            p->frame.ref_pic_id[LIST_1][j][i] = __c64(-1,-1);
          }
          else
          {
