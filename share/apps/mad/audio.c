 ...
 *
 * $Id: audio.c,v 1.2 2009/12/17 11:04:43 frb Exp $
 */
...
extern int xstrncasecmp(char const *, char const *, unsigned long);
extern int xstrcasecmp(char const *, char const *);
...
# endif
  ...
  if (ext) {
    ...
    *path = ext + 1;
    ...
    for (i = 0; i < sizeof(types) / sizeof(types[0]); ++i) {
      if (xstrncasecmp(type, types[i].name, ext - type) == 0 &&
  ...
  if (ext) {
    ...
    for (i = 0; i < sizeof(exts) / sizeof(exts[0]); ++i) {
      if (xstrcasecmp(ext, exts[i].name) == 0)
