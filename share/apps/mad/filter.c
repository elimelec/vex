 ...
 *
 * $Id: filter.c,v 1.1 2009/12/17 10:32:00 frb Exp $
 */
...
enum mad_flow fadein_filter(void *data, struct mad_frame *frame)
{
  ...
  mad_timer_t fadein = player->fade_in;

  if (mad_timer_compare(player->stats.play_timer, fadein) < 0) {
    ...
    if (mad_timer_compare(fadein, frame_end) < 0) {
      mad_timer_t length;

      length = frame_start;

      mad_timer_negate(&length);
      mad_timer_add(&length, fadein);
